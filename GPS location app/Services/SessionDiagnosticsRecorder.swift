...
final class SessionDiagnosticsRecorder: ObservableObject {
    ...
    struct Row {
        ...
        /// Mean absolute error, m/s, of predicting the query's near neighbours from each other.
        /// Large means the stored labels around this signature disagree, so the weighted mean of
        /// them is an average of contradictions rather than a measurement.
        ...
        /// Real GPS fixes appended to the drawn route so far. In Velocity Mode this must reach
        /// 1 and stay there; the same workout reached 63.
        let gpsFixesInRoute: Int
        /// 1 while the barometer and gyro together say this is a car-park ramp - climbing
        /// steadily while turning continuously the same way - and distance is being withheld
        /// because the vibration model reads a crawl on concrete as 40 km/h.
        let onRamp: Bool
        let handlingRotation: Double
        /// Seconds the device has been continuously in a hand, 0 when resting in the vehicle.
        /// Past HANDLING_MEANS_DISMOUNTED the held vehicle speed is refused — this column is how
        /// a log shows that happening rather than leaving it to be inferred from a source tag.
        let handledSeconds: Double
        /// NO USABLE HEADING.
        ///
        ...
        let altitude: Double?
        /// Both speed engines at this tick, whichever was driving: the bundled network and the
        /// learned store, in m/s, blank when that engine declined or cannot answer. speed_engine
        /// names the one the estimate used ("network" or "store"), so a log can compare the other
        /// on exactly the same seconds.
        let networkSpeed: Double?
        let storeSpeed: Double?
        let speedEngine: String
        /// Speed gained since the phone was last still (LaunchIntegrator), m/s; used only in the air.
    ...
    func csv() -> String {
        ...
        out += "net_speed_ms,store_speed_ms,speed_engine,launch_speed_ms\n"
        ...
        for r in rows {
            ...
            out += Self.fmt(r.networkSpeed) + "," + Self.fmt(r.storeSpeed) + "," + Self.csvField(r.speedEngine) + "," + Self.fmt(r.launchSpeed) + "\n"
        }
        return out
    }
