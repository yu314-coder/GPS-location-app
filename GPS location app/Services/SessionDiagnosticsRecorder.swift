...
final class SessionDiagnosticsRecorder: ObservableObject {
    ...
    func csv() -> String {
        ...
        out += "tick_dt_s,regime_distance,regime_declined,local_error_ms,local_declined,offset_warmup,velocity_mode,gps_fixes_in_route,on_ramp,handling_rot_rads,heading_unreliable,walk_axis_deg,walk_skew_ema,walk_axis_raw_deg,walk_axis_gated,"
        ...
        for r in rows {
            ...
            out += Self.fmt(r.tickInterval) + "," + (r.regimeDistance.map { Self.fmt($0) } ?? "") + ",\(r.regimeDeclined ? 1 : 0)," + (r.localError.map { Self.fmt($0) } ?? "") + ",\(r.localDeclined ? 1 : 0),\(r.offsetWarmup ? 1 : 0),\(r.velocityMode ? 1 : 0),\(r.gpsFixesInRoute),\(r.onRamp ? 1 : 0)," + Self.fmt(r.handlingRotation) + ",\(r.headingUnreliable ? 1 : 0),"
