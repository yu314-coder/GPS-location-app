...
struct SpeedSample: Codable, Hashable {
    let timestamp: Date
...
struct PressureSample: Codable, Hashable {
    let timestamp: Date
...
struct FlightMetrics: Codable, Hashable {
    ...
    mutating func updateWithLocation(_ location: FlightLocation, previousLocation: FlightLocation?, elapsedTime: TimeInterval = 0) {
        ...
        if let previous = previousLocation {
            ...
            // Log altitude changes every 25 points for monitoring
            if totalPoints % 25 == 0 {
                print("⛰️  Altitude: \(String(format: "%.1f", location.altitude))m (Δ\(String(format: "%+.1f", altitudeDelta))m) | Gain: \(String(format: "%.1f", totalAltitudeGain))m | Loss: \(String(format: "%.1f", totalAltitudeLoss))m | Max: \(String(format: "%.1f", maxAltitude))m | Min: \(String(format: "%.1f", minAltitude))m")
            }
            ...
            if timeDelta > 0 && distance > 0 {
                ...
                if totalPoints % 25 == 0 {
                    print("⚡ Speed calc: distance=\(String(format: "%.2f", distance))m, time=\(String(format: "%.2f", timeDelta))s → \(String(format: "%.1f", currentSpeed * 3.6))km/h")
                }
            } else if timeDelta > 0 {
                currentSpeed = 0.0
            }
            ...
            if smoothedSpeed == 0.0 {
                ...
                let previousSmoothed = smoothedSpeed
                smoothedSpeed = alpha * currentSpeed + (1.0 - alpha) * smoothedSpeed

                // Log smoothing every 25 points
                if totalPoints % 25 == 0 {
                    print("   Smoothed: \(String(format: "%.1f", previousSmoothed * 3.6))km/h → \(String(format: "%.1f", smoothedSpeed * 3.6))km/h")
                }
            }
        } else {
            currentSpeed = 0.0
        ...
        if currentSpeed > maxSpeed {
            ...
            // Log max speed updates for debugging
            if totalPoints % 50 == 0 || maxSpeed > oldMax * 1.1 { // Log if >10% increase
                print("🏎️  NEW MAX SPEED: \(String(format: "%.1f", oldMax * 3.6))km/h → \(String(format: "%.1f", maxSpeed * 3.6))km/h (instantaneous)")
            }
        }
        ...
        if elapsedTime > 0 {
            ...
            // Log average speed calculation every 50 points
            if totalPoints % 50 == 0 {
                print("📊 Avg speed: \(String(format: "%.2f", totalDistance))m ÷ \(String(format: "%.1f", elapsedTime))s = \(String(format: "%.1f", averageSpeed * 3.6))km/h")
            }
        }
