...
class HealthKitManager: ObservableObject {
    ...
    private func distanceSampleSources(from metrics: FlightMetrics) -> [(label: String, value: Double)] {
        var sources: [(label: String, value: Double)] = []
        if metrics.totalDistance > 0 {
            sources.append(("gps", metrics.totalDistance))
        }
        if let nativeStepDistance = metrics.nativeStepDistance, nativeStepDistance > 0 {
            sources.append(("nativeStep", nativeStepDistance))
        }
        return sources
    }

    private func distanceSampleMetadata(source: String) -> [String: Any] {
        var metadata = autoGeneratedSampleMetadata
        metadata["com.exmstc.gps.distanceSource"] = source
        metadata["com.exmstc.gps.isGPSDistance"] = source == "gps"
        metadata["com.exmstc.gps.isNativeStepDistance"] = source == "nativeStep"
        return metadata
    }

    private func primaryDistanceIdentifier(for activityType: HKWorkoutActivityType) -> HKQuantityTypeIdentifier {
        switch activityType {
        case .cycling:
    ...
    func saveWorkoutWithResult(
        ...
        builder.beginCollection(withStart: flight.startDate) { success, error in
            ...
            builder.addMetadata(sanitizedHealthKitMetadata(metadata)) { success, error in
                guard success else {
                    completion(false, error, nil)
                    return
                }
                ...
                self.addWorkoutSamples(
                    ...
                    self.addCaloriesAndStepsSamples(
                        ...
                        // End collection before adding distance. HKLiveWorkoutDataSource can own distance
                        // during collection, so GPS and native step distances are associated post-finish.
        ...
        if metrics.caloriesBurned > 0 {
            if let energyType = HKQuantityType.quantityType(forIdentifier: .activeEnergyBurned) {
                let energySample = HKQuantitySample(
                    type: energyType,
                    quantity: HKQuantitySafe(unit: .kilocalorie(), doubleValue: metrics.caloriesBurned),
                    start: startDate,
                    end: endDate,
        ...
        if metrics.restingEnergyBurned > 0 {
            if let basalType = HKQuantityType.quantityType(forIdentifier: .basalEnergyBurned) {
                let basalSample = HKQuantitySample(
                    type: basalType,
                    quantity: HKQuantitySafe(unit: .kilocalorie(), doubleValue: metrics.restingEnergyBurned),
                    start: startDate,
                    end: endDate,
                    metadata: autoGeneratedSampleMetadata
                )
    ...
    func addDistanceSamplesIfMissing(
        ...
        let distanceSources = distanceSampleSources(from: metrics)
        guard !distanceSources.isEmpty else {
            completion()
            return
        }
        ...
        for source in distanceSources {
            for identifier in distanceIdentifiers {
                ...
                let distanceSample = HKQuantitySample(
                    ...
                    metadata: distanceSampleMetadata(source: source.label)
                )
    ...
    func addWorkoutTotalDistanceSample(
        to builder: HKWorkoutBuilder,
        metrics: FlightMetrics,
        startDate: Date,
        endDate: Date,
        ...
        builder.add([sample]) { success, error in
            if success {
                ...
                print("⌚ ❌ Failed to add official workout distance sample: \(error?.localizedDescription ?? "Unknown")")
            }
            completion(success)
        }
    }

    // MARK: - Post-Finish Distance Association
    // HKLiveWorkoutDataSource auto-collects .distanceWalkingRunning for running/walking,
    // which blocks builder.add() for the same type. Write GPS distance AFTER finishWorkout
    // using healthStore.add(_:to:) so it bypasses the live data source conflict.

    func associateDistanceSamples(
        to workout: HKWorkout,
        metrics: FlightMetrics,
        startDate: Date,
        endDate: Date,
        activityType: HKWorkoutActivityType,
        includeGPSDistance: Bool = true,
        includeNativeStepDistance: Bool = true,
        completion: @escaping (Bool, Error?) -> Void
    ) {
        let distanceSources = distanceSampleSources(from: metrics).filter { source in
            switch source.label {
            case "gps":
                return includeGPSDistance
            case "nativeStep":
                return includeNativeStepDistance
            default:
                return true
            }
        }
        guard !distanceSources.isEmpty else {
            print("⌚ ⚠️ No GPS/native distance to associate")
            completion(true, nil)
            return
        }
        ...
        let sourceText = distanceSources.map { "\($0.label)=\(String(format: "%.2f", $0.value))m" }.joined(separator: ", ")
        print("⌚ 📏 Associating distance samples post-finish [\(sourceText)] to workout \(workout.uuid)")

        healthStore.add(distanceSamples, to: workout) { success, error in
            if success {
                print("⌚ ✅ GPS/native distance samples associated with workout - will appear in Fitness app")
            } else {
                print("⌚ ❌ Failed to associate distance samples: \(error?.localizedDescription ?? "Unknown")")
            }
            completion(success, error)
        }
    }
    ...
    func addDistanceAndEnergySamples(
        ...
        if let stepType = HKQuantityType.quantityType(forIdentifier: .stepCount) {
            if let actualSteps = metrics.stepsCount, actualSteps > 0 {
                let stepSample = HKQuantitySample(
                    type: stepType,
                    quantity: HKQuantitySafe(unit: .count(), doubleValue: actualSteps),
                    start: startDate,
                    end: endDate,
                    metadata: autoGeneratedSampleMetadata
                )
